          ...
          <div class="card">
            <div class="card-header">
              <h3 class="card-title">Whitelist</h3>
                ...
                <a href="{{route('whitelist.add')}}" class="btn btn-primary btn-block">Add</a>
        ...
        buttons: [
            {
                ...
                action: function ( e, dt, node, config ) {
                    if(confirm( 'Are you sure do you want to delete selected items ?' )){
                      ...
                      $form = $("<form action='{{route('whitelist.bulk_delete')}}' method='post'></form>");
                      $form.append('@csrf');
            ...
            },
            {
                ...
                action: function ( e, dt, node, config ) {
                  if(confirm( 'Are you sure do you want to disable selected items ?' )){
                    var rows_selected = table.column(0).checkboxes.selected();
                      $form = $("<form action='{{route('whitelist.bulk_disable')}}' method='post'></form>");
                      $form.append('@csrf');
                      $.each(rows_selected, function(index, rowId){
                        $form.append(
             $('<input>')
            ...
            },
            {
                ...
                action: function ( e, dt, node, config ) {
                  if(confirm( 'Are you sure do you want to Activate selected items ?' )){
                    var rows_selected = table.column(0).checkboxes.selected();
                      $form = $("<form action='{{route('whitelist.bulk_activate')}}' method='post'></form>");
                      $form.append('@csrf');
                      $.each(rows_selected, function(index, rowId){
                        $form.append(
             $('<input>')
