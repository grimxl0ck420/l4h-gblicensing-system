          ...
          <div class="card">
            <div class="card-header">
              <h3 class="card-title">Whitelist</h3>
                ...
                <a href="<?php echo e(route('whitelist.add')); ?>" class="btn btn-primary btn-block">Add</a>
...
<script type="text/javascript">
  $(function () {
    ...
    var table = $('.data-table').DataTable({
        ...
        "lengthChange": false,
        "autoWidth": false,
        ajax: "<?php echo e(route('whitelist')); ?>",
        'select': {
         'style': 'multi'
      },
      dom: 'Bfrtip',
        buttons: [
            {
                ...
                action: function ( e, dt, node, config ) {
                    if(confirm( 'Are you sure do you want to delete selected items ?' )){
                      ...
                      $form = $("<form action='<?php echo e(route('whitelist.bulk_delete')); ?>' method='post'></form>");
            ...
            },
            {
                ...
                action: function ( e, dt, node, config ) {
                  if(confirm( 'Are you sure do you want to disable selected items ?' )){
                    var rows_selected = table.column(0).checkboxes.selected();
                      $form = $("<form action='<?php echo e(route('whitelist.bulk_disable')); ?>' method='post'></form>");
                      $form.append('<?php echo csrf_field(); ?>');
                      $.each(rows_selected, function(index, rowId){
                        $form.append(
             $('<input>')
            ...
            },
            {
                ...
                action: function ( e, dt, node, config ) {
                  if(confirm( 'Are you sure do you want to Activate selected items ?' )){
                    var rows_selected = table.column(0).checkboxes.selected();
                      $form = $("<form action='<?php echo e(route('whitelist.bulk_activate')); ?>' method='post'></form>");
                      $form.append('<?php echo csrf_field(); ?>');
                      $.each(rows_selected, function(index, rowId){
                        $form.append(
             $('<input>')
...
<?php echo $__env->make('layouts.layout', \Illuminate\Support\Arr::except(get_defined_vars(), ['__data', '__path']))->render(); ?><?php /**PATH /var/www/vhosts/license4.host/api.license4.host/application/resources/views/whitelist.blade.php ENDPATH**/ ?>